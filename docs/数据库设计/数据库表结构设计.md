...
说明：用于保存普通注册用户和管理员账号信息，其中 role 字段用于区分普通用户和管理员。

2. `students`：学生信息表如表 4-2 所示。

**表 4-2 学生信息表 `students`**
...
说明：用于保存管理员批量导入的学生账号信息，学生使用学号登录系统。

3. `teachers`：教师信息表如表 4-3 所示。

**表 4-3 教师信息表 `teachers`**
...
说明：用于保存管理员批量导入的教师账号信息，教师使用工号登录系统。

4. `publishers`：出版社信息表如表 4-4 所示。

**表 4-4 出版社信息表 `publishers`**
...
说明：用于保存图书出版社基础信息，便于维护图书来源数据。

5. `books`：图书信息表如表 4-5 所示。
...
说明：用于保存图书馆馆藏图书的基础信息、馆藏位置、借阅状态和封面信息。

6. `borrow_records`：借阅记录表如表 4-6 所示。

**表 4-6 借阅记录表 `borrow_records`**
...
说明：用于保存用户借阅和归还图书的记录，支持普通用户、学生和教师三类账号。

7. `seats`：座位信息表如表 4-7 所示。
...
说明：用于保存图书馆座位的楼层、区域、编号、电源配置和使用状态。

8. `seat_reservations`：座位预约记录表如表 4-8 所示。

**表 4-8 座位预约记录表 `seat_reservations`**
...
说明：用于保存用户预约和释放座位的记录，支持普通用户、学生和教师三类账号。

9. `drift_books`：漂流图书信息表如表 4-9 所示。

**表 4-9 漂流图书信息表 `drift_books`**
...
说明：用于保存用户发布的漂流图书信息，以及提供者、领取者和漂流状态。

10. `drift_requests`：漂流图书申请表如表 4-10 所示。

**表 4-10 漂流图书申请表 `drift_requests`**
...
说明：用于保存用户对漂流图书提交的领取申请及申请处理状态。

11. `book_favorites`：图书收藏表如表 4-11 所示。

**表 4-11 图书收藏表 `book_favorites`**
...
说明：用于保存用户收藏图书的记录，支持普通用户、学生和教师三类账号。

12. `book_reviews`：图书评价表如表 4-12 所示。

**表 4-12 图书评价表 `book_reviews`**
...
说明：用于保存用户对图书提交的评价内容、评分和 AI 审核状态。
